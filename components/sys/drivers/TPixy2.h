...
/*
  ...
  Pixy2CCC ccc;
  friend class Pixy2CCC;

  // Line following
  Pixy2Line line;
  friend class Pixy2Line;

  // Video
  Pixy2Video video;
  friend class Pixy2Video;
*/
  Pixy2Link m_link;
...
TPixy2::TPixy2()
   : m_type(0), m_length(0), m_cs(0)
{
  // allocate buffer space for send/receive
  m_buf = (uint8_t *)malloc( (size_t)PIXY_BUFFERSIZE );
...
int8_t TPixy2::init(uint32_t arg)
{
  uint32_t t0;
  int8_t res;
  ...
  for(t0=esp_timer_get_time(); esp_timer_get_time()-t0<5000*1000; )
  {
...
int16_t TPixy2::getSync()
{
  ...
  for(i=j=0, cprev=0; true; i++)
  {
    ...
    if (i>=4)
    {
      ...
      {
#ifdef PIXY_DEBUG
        printf("TPixy2: error: no response\r\n");
#endif		  
...
int16_t TPixy2::recvPacket()
{
  ...
  if (m_cs)
  {
    ...
    //printf("recv type:%x, bytes:%d sum:%x", m_type, m_length, csSerial);
    ...
    {
#ifdef PIXY_DEBUG
      printf("TPixy2: error: checksum %d got %d\r\n", csSerial, csCalc);
#endif
...
int8_t TPixy2::changeProg(const char *prog)
{
  int32_t res;
...
int8_t TPixy2::setCameraBrightness(uint8_t brightness)
{
  uint32_t res;
...
int8_t TPixy2::setServos(uint16_t s0, uint16_t s1)
{
  uint32_t res;
...
int8_t TPixy2::setLED(uint8_t r, uint8_t g, uint8_t b)
{
  uint32_t res;
...
int8_t TPixy2::setLamp(uint8_t upper, uint8_t lower)
{
  uint32_t res;
