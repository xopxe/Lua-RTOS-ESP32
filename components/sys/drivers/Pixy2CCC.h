...
struct Block 
{
  ...
  void print()
  {
    ...
    if (m_signature>CCC_MAX_SIGNATURE) // color code! (CC)
    {
      ...
      sig[j] = '\0';  
      printf("Pixy2CCC: CC block sig: %s (%d decimal) x: %d y: %d width: %d height: %d angle: %d index: %d age: %d\n", sig, m_signature, m_x, m_y, m_width, m_height, m_angle, m_index, m_age);
    }     
    else // regular block.  Note, angle is always zero, so no need to print
      printf("Pixy2CCC: sig: %d x: %d y: %d width: %d height: %d index: %d age: %d\n", m_signature, m_x, m_y, m_width, m_height, m_index, m_age);
  }
...
{
public:
  Pixy2CCC(TPixy2 *pixy):numBlocks(0), blocks(NULL)
  {
    m_pixy = pixy;
  }
