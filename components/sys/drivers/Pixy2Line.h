...
struct Vector
{
  void print()
  {
    printf("Pixy2Line: vector: (%d %d) (%d %d) index: %d flags %d\n", m_x0, m_y0, m_x1, m_y1, m_index, m_flags);
  }
...
struct Intersection
{
  ...
  {
    uint8_t i;
    printf("Pixy2Line: Intersection: (%d %d)\n", m_x, m_y);
    for (i=0; i<m_n; i++)
    {
      printf("Pixy2Line:   branch %d: index: %d angle: %d\n", i, m_intLines[i].m_index, m_intLines[i].m_angle);
    }
  }
...
struct Barcode
{
  void print()
  {
    printf("Pixy2Line: Barcode: (%d %d), val: %d flags: %d\n", m_x, m_y, m_code, m_flags);
  }
...
int8_t Pixy2Line::getFeatures(uint8_t type,  uint8_t features, bool wait)
{
  int8_t res;
...
int8_t Pixy2Line::setMode(uint8_t mode)
{
  uint32_t res;
...
int8_t Pixy2Line::setNextTurn(int16_t angle)
{
  uint32_t res;
...
int8_t Pixy2Line::setDefaultTurn(int16_t angle)
{
  uint32_t res;
...
int8_t Pixy2Line::setVector(uint8_t index)
{
  uint32_t res;
...
int8_t Pixy2Line::reverseVector()
{
  uint32_t res;
