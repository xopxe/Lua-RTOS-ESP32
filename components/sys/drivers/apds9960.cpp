...
#include "sdkconfig.h"
#if CONFIG_LUA_RTOS_LUA_USE_APDS9960 || CONFIG_LUA_RTOS_LUA_USE_APDS9960_ROBOTITO
...
{
    printf("In: SparkFun_APDS9960::init()\r\n");

    driver_error_t *error;

	uint8_t i2c = CONFIG_APDS9960_I2C_CHANNEL;
	if ((error=i2c_attach(i2c, I2C_MASTER, CONFIG_APDS9960_SPEED, 0, 0, &i2cdevice))) {
...
bool SparkFun_APDS9960::setMode(uint8_t mode, uint8_t enable)
{
    ...
    enable = enable & 0x01;
    //if( mode >= 0 && mode <= 6 ) {  //XOP mode>=uint...
    if( mode <= 6 ) {
        if (enable) {
            reg_val |= (1 << mode);
        } else {
...
{
    i2c_util_writeReg(i2cdevice, address, reg, val);
    return true;
}
...
{
    val = i2c_util_readReg(i2cdevice, address, reg);
    return true;
}
